...
app.post('/driver-signup', (req, res) => {
  ...
  connection.query(insertSql, [username, phone, matricule], async (err, result) => {
    if (err) {
    ...
    // Generate QR Code with "driver:<id>" format
    try {
      const qrData = `driver:${newDriverId}`;
      const qrImageBuffer = await QRCode.toBuffer(qrData);

      // Save QR code blob to the database
      const updateQRSql = 'UPDATE driver SET qr_code = ? WHERE id = ?';
      connection.query(updateQRSql, [qrImageBuffer, newDriverId], (qrErr) => {
        if (qrErr) {
          console.error('Error saving QR code:', qrErr);
          // Continue even if QR code saving fails
        }

        // Save session after QR is generated
        req.session.user = {
          type: 'driver',
          id: newDriverId,
          username,
        ...
        res.redirect('/driver_profile.html');
      });
    } catch (qrGenError) {
      console.error('QR code generation error:', qrGenError);
...
app.get('/driver/:id/qr', (req, res) => {
  ...
  connection.query(sql, [driverId], (err, results) => {
    ...
    const qrBuffer = results[0].qr_code;
